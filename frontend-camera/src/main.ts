...
import { PairingRequest } from "../bindings/github.com/zecloud/ai-video-studio/internal/dji/models.js";
...
function render(): void {
  ...
  app.innerHTML = `<main class="shell"><header><div><p class="eyebrow">DJI hardware boundary</p><h1>AI Video Camera</h1><p class="lede">Connect, pair, and validate the Osmo Action camera. Transfers and cloud workflows live in AI Video Studio.</p></div><span class="pill">${esc(status)}</span></header>
    <section class="grid"><article class="panel"><div class="panel-head"><div><p class="eyebrow">BLE / DUML profile</p><h2>${esc(protocol?.modelHint || "Osmo Action 4")}</h2></div><button data-action="diagnose">Run diagnostics</button></div>
      ...
      <article class="panel"><div class="panel-head"><div><p class="eyebrow">Windows Bluetooth</p><h2>Nearby devices</h2></div><button data-action="scan">Scan BLE</button></div><div class="devices">${devices.length ? devices.map((device) => `<button class="device ${device.id === selected ? "selected" : ""}" data-device="${esc(device.id)}"><strong>${esc(device.name || "Unnamed peripheral")}</strong><span>${esc(device.model || "BLE peripheral")} · RSSI ${device.rssi}</span><small>${esc(device.address || device.id)}</small></button>`).join("") : `<p class="empty">No scan results. Put the camera in wireless/app control mode, then scan.</p>`}</div><button class="secondary wide" data-action="pair" ${selected ? "" : "disabled"}>Pair / GATT test</button></article></section>
    <section class="panel"><div class="panel-head"><div><p class="eyebrow">HTTP media readiness</p><h2>Camera endpoint</h2></div><button class="secondary" data-action="probe">Probe HEAD + Range</button></div><div class="endpoint"><label>Camera IP<input id="ip" value="${esc(protocol?.defaultIp || "192.168.2.1")}" /></label><label>Media path<input id="path" value="/v2?storage=1&amp;path=%2FDCIM%2F100MEDIA" /></label></div><p class="message" aria-live="polite">${esc(message)}</p></section>
...
async function probe(): Promise<void> { const ip = app.querySelector<HTMLInputElement>("#ip")?.value || "192.168.2.1"; const path = app.querySelector<HTMLInputElement>("#path")?.value || "/v2"; message = "Probing endpoint..."; render(); try { const result = await CameraService.ProbeEndpoint(new EndpointProbeRequest({ ipAddress: ip, path, storage: CameraStorage.CameraStorageSD })); message = result.message; status = result.rangeOk ? "Media ready" : "Endpoint reachable"; } catch (error) { message = error instanceof Error ? error.message : "Endpoint probe failed."; } render(); }
render();
